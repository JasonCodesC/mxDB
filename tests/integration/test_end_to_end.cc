#include <cassert>
#include <chrono>
#include <filesystem>
#include <string>
#include <vector>

#include "engine/catalog/metadata_store.h"
#include "engine/recovery/recovery_manager.h"
#include "engine/storage/feature_engine.h"

namespace {

mxdb::TimestampMicros NowMicros() {
  const auto now = std::chrono::system_clock::now();
  return std::chrono::duration_cast<std::chrono::microseconds>(
             now.time_since_epoch())
      .count();
}

std::filesystem::path UniqueTmpDir(const std::string& name) {
  const auto base = std::filesystem::temp_directory_path();
  const auto stamp = std::to_string(NowMicros());
  std::filesystem::path path = base / ("mxdb-" + name + "-" + stamp);
  std::filesystem::create_directories(path);
  return path;
}

mxdb::EntityFeatureBatch MakeDoubleEvent(const std::string& entity_id,
                                         const std::string& feature_id,
                                         const std::string& write_id,
                                         double value,
                                         mxdb::TimestampMicros event_time_us,
                                         mxdb::TimestampMicros system_time_us) {
  mxdb::EntityFeatureBatch batch;
  batch.entity = {.tenant_id = "prod", .entity_type = "instrument", .entity_id = entity_id};
  mxdb::FeatureEventInput input;
  input.feature_id = feature_id;
  input.event_time_us = event_time_us;
  input.system_time_us = system_time_us;
  input.value = {.type = mxdb::ValueType::kDouble, .value = value};
  input.operation = mxdb::OperationType::kUpsert;
  input.write_id = write_id;
  input.source_id = "integration";
  batch.events.push_back(std::move(input));
  return batch;
}

}  // namespace

int main() {
  const auto tmp = UniqueTmpDir("e2e");

  mxdb::EngineConfig config;
  config.data_dir = tmp.string();
  config.metadata_path = (tmp / "catalog" / "metadata.db").string();
  config.wal_dir = (tmp / "wal").string();
  config.partition_count = 4;

  mxdb::MetadataStore metadata;
  mxdb::Status status = metadata.Open(config.metadata_path);
  assert(status.ok());

  mxdb::FeatureDefinition feature;
  feature.tenant_id = "prod";
  feature.feature_id = "f_price";
  feature.feature_name = "price";
  feature.entity_type = "instrument";
  feature.value_type = mxdb::ValueType::kDouble;
  feature.serving_enabled = true;
  feature.historical_enabled = true;
  feature.allow_external_system_time = true;
  feature.nullable = false;
  feature.description = "price";
  feature.owner = "team";
  feature.created_at_us = NowMicros();
  feature.updated_at_us = feature.created_at_us;

  status = metadata.CreateFeature(feature);
  assert(status.ok());

  {
    mxdb::FeatureEngine engine(config, &metadata);
    status = engine.Start();
    assert(status.ok());

    auto r1 = engine.WriteEntityBatch(
        MakeDoubleEvent("AAPL", "f_price", "w1", 10.0, 100, 110),
        mxdb::DurabilityMode::kSync, true);
    assert(r1.ok());

    auto r2 = engine.WriteEntityBatch(
        MakeDoubleEvent("AAPL", "f_price", "w2", 12.0, 110, 120),
        mxdb::DurabilityMode::kSync, true);
    assert(r2.ok());

    auto latest = engine.GetLatest(
        {.tenant_id = "prod", .entity_type = "instrument", .entity_id = "AAPL"},
        {"f_price"});
    assert(latest.ok());
    assert(latest.value().features[0].found);
    assert(std::get<double>(latest.value().features[0].value.value) == 12.0);

    status = engine.Stop();
    assert(status.ok());
  }

  {
    mxdb::FeatureEngine recovered_engine(config, &metadata);
    status = recovered_engine.Start();
    assert(status.ok());

    mxdb::RecoveryManager recovery(&recovered_engine);
    bool had_truncated_tail = false;
    status = recovery.RecoverFromWalDirectory(config.wal_dir, &had_truncated_tail);
    assert(status.ok());
    assert(!had_truncated_tail);

    auto as_of = recovered_engine.AsOfLookup(
        {.entity = {.tenant_id = "prod", .entity_type = "instrument", .entity_id = "AAPL"},
         .feature_ids = {"f_price"},
         .event_cutoff_us = 115,
         .system_cutoff_us = 200});
    assert(as_of.ok());
    assert(as_of.value().features[0].found);
    assert(std::get<double>(as_of.value().features[0].value.value) == 12.0);

    auto dedupe = recovered_engine.WriteEntityBatch(
        MakeDoubleEvent("AAPL", "f_price", "w2", 99.0, 999, 999),
        mxdb::DurabilityMode::kSync, true);
    assert(dedupe.ok());
    assert(dedupe.value().accepted_events == 0);

    std::vector<mxdb::DrivingRow> rows;
    rows.push_back({.row_id = "row-1",
                    .entity = {.tenant_id = "prod", .entity_type = "instrument", .entity_id = "AAPL"},
                    .label_time_us = 105,
                    .system_cutoff_us = 115});
    rows.push_back({.row_id = "row-2",
                    .entity = {.tenant_id = "prod", .entity_type = "instrument", .entity_id = "AAPL"},
                    .label_time_us = 115,
                    .system_cutoff_us = 130});

    auto dataset = recovered_engine.BuildTrainingDataset(rows, {"f_price"}, 130);
    assert(dataset.ok());
    assert(dataset.value().size() == 2);
    assert(std::get<double>(dataset.value()[0].at("f_price").value.value) == 10.0);
    assert(std::get<double>(dataset.value()[1].at("f_price").value.value) == 12.0);

    status = recovered_engine.Stop();
    assert(status.ok());
  }

  status = metadata.Close();
  assert(status.ok());

  std::filesystem::remove_all(tmp);
  return 0;
}
