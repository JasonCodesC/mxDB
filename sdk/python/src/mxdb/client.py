...
class MXDBClient:
    def __init__(
        self, config_path: Optional[str] = None, featurectl_bin: Optional[str] = None
    ) -> None:
        if config_path is None:
            resolved_config = self._ensure_default_config_path()
        else:
    ...
    def _default_home_dir() -> Path:
        env_home = os.environ.get("MXDB_HOME")
        if env_home:
            return Path(env_home).expanduser()
        return Path.home() / ".mxdb"

    @classmethod
    def _ensure_default_config_path(cls) -> Path:
        root = cls._default_home_dir()
        config_path = root / "featured.conf"
        if config_path.exists():
            if config_path.is_dir():
                raise RuntimeError(f"default config path is a directory: {config_path}")
            return config_path
        ...
        config_text = "\n".join(
            [
                "# auto-generated by mxdb Python SDK",
                f"data_dir={data_dir}",
                ...
                "",
                "partition_count=8",
                "memtable_flush_event_threshold=2048",
                "",
                "wal_segment_target_bytes=16777216",
                "wal_group_commit_max_records=1024",
                "wal_group_commit_window_ms=5",
                "",
                "# true => sync mode, false => group commit mode",
                "default_durability_sync=false",
            ]
        )
